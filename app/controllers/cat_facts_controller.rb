...
#   @cat_fact = response.success? && response['fact'].present? ? response['fact'] : "Erro ao buscar fato" #=> @cat_fact -> variável de instância usada na view. response.success? -> checa se a requisição foi bem sucedida. response['fact'] -> extrai o fato do JSON. Se algo der errado, mostra "Erro ao buscar fato".
    @cat_facts = Fact.where(created_at: 1.day.ago..Time.zone.now).limit(10).offset(10)
end
end